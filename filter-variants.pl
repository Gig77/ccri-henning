...
while (my $line = $vcf->next_line())
{
	...
	if ($x->{INFO}{'SS'} == 0 or $x->{INFO}{'SS'} == 1) # reference or germline?
	{
		$filtered_germ ++;
		next;	
	}
	...
	if ($x->{INFO}{'SS'} == 2 and $ad_rem_alt > 0) { $reject = 1; push(@reject_because, "present normal"); };
	if ($x->{INFO}{'SS'} == 2 and !$both_strands) { $reject = 1; push(@reject_because, "single strand"); };
	if ($x->{INFO}{'SS'} == 5) { $reject = 1; push(@reject_because, "unknown somatic status"); };
